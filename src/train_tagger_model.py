#!/usr/bin/env python
"""Train wrapper for tagger model."""
...
from .model.tagger_model_helper import train_model
from .model.configure import TaggerModelConfiguration
from .model.configure import UserModelConfig
...
def main():
    ...
    cmdline_parser.add_argument('--batch_size',
                                ...
                                ' for the model)')
    cmdline_parser.add_argument('--pretrain_model_dir',
                                default=None,
                                help='loads pretrain unsupervised model(s)')
    cmdline_parser.add_argument('--pretrain_model_config',
                                default=None,
                                help='loads pretrain unsupervised model config')
    ...
    cmdline_parser.add_argument('--num_dialog_act',
                                type=int,
                                default=0,
                                help='number of dialog acts.')
    cmdline_parser.add_argument('--dialog_act_embed_dim',
                                type=int,
                                default=0,
                                help='dialog act embedding dimension.')
    ...
    cmdline_parser.add_argument('--adapt_user_model',
                                type=int,
                                default=1,
                                help='whether to adapts the user model.')
    ...
    rnn_param_prefix_set = ['user_state', 'user_utterance',
    ...
    args_dict = vars(args)
    train_config = TaggerModelConfiguration()
    train_config.parse_from_dict(args_dict)

    train_config.save_model_dir = args.model_outdir

    if args.pretrain_model_dir:
        if not args.pretrain_model_config:
            print('When pretrain_model_dir is specified,',
                  'pretrain_model_config is required', file=sys.stderr)
            sys.exit(1)

        train_config.pretrain_model_dir = args.pretrain_model_dir
        pretrain_model_config = UserModelConfig()
        pretrain_model_config.parse_from_txt_file(args.pretrain_model_config)
        print('Reads UserModelConfig from {0}'.format
              (args.pretrain_model_config), file=sys.stderr)
        train_config.model_config.update_user_model_related_parameter(
    ...
    train_model(
        ...
        valid_filename=args.validation_data_filename,
